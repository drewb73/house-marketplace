...
function Listing() {
  ...
  return (
    <main>
        <Swiper  modules={[Navigation, Pagination, Scrollbar, A11y]} slidesPerView={1} pagination={{clickable: true}} style={{ height: '300px' }} >
             ...
             <MapContainer style={{height: '100%', width:'100%'}} center={[listing.geolocation.lat, listing.geolocation.lng]} zoom={13} scrollWheelZoom={false}>
              ...
              url='https://{s}.tile.openstreetmap.de/tiles/osmde/{z}/{x}/{y}.png' />
                ...
                <Marker position={[listing.geolocation.lat, listing.geolocation.lng]}>
                    <Popup>{listing.name}<br></br>{listing.location}<br></br>${listing.regularPrice.toString()
                    .replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Popup>
                </Marker>
             </MapContainer>
